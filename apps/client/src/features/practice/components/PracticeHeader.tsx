...
type PracticeHeaderProps = {
  ...
  isComplete: boolean;
  tempo: number;
  isControlRailExpanded: boolean;
  onToggleControlRail: () => void;
...
export const PracticeHeader = ({
  ...
  <header className="practice-control-panel mx-auto grid w-full max-w-7xl gap-5 px-4 pt-5 md:px-6 lg:grid-cols-[1fr_auto] lg:items-center">
    <button
      type="button"
      className="practice-rail-toggle hidden"
      aria-label={isControlRailExpanded ? "Collapse controls" : "Expand controls"}
      aria-expanded={isControlRailExpanded}
      onClick={onToggleControlRail}
    >
      <span aria-hidden="true">{isControlRailExpanded ? "<" : ">"}</span>
      <span className="practice-rail-toggle-label">Controls</span>
    </button>
    <div className="practice-rail-content contents">
        ...
        <div className="practice-select-row flex flex-wrap items-center justify-end gap-2">
          ...
          <label className="practice-pill-control flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm text-zinc-300">
            ...
            <select
              value="A minor"
              disabled
              className="bg-transparent text-sm font-semibold text-zinc-300 outline-none"
            >
