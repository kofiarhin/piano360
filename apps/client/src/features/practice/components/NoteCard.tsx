...
export const NoteCard = ({ noteId, index, currentIndex, result, feedback }: NoteCardProps) => {
  ...
  return (
    <motion.div
      ...
      className={[
        "practice-note-card relative flex h-36 w-24 shrink-0 flex-col items-center justify-center rounded-2xl border text-center shadow-2xl md:h-40 md:w-28",
        isCurrent
    ...
    >
      ...
      {isCurrent && (
        <motion.span
          ...
          className="practice-now-label absolute -bottom-9 text-xs font-black uppercase text-violet-200"
        >
          PRESS NOW
        </motion.span>
      )}
